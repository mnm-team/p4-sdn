...
class ARPCache(threading.Thread):
    def __init__(self):
        ...
        #self.con = {1:APPCore('s1'), 2:APPCore('s2'), 3:APPCore('s3'), 4:APPCore('s4'), 5:APPCore('s5')}
        self.q = None
    ...
    def init(self):
        ...
        #self.q = {1:self.con[1].q, 2:self.con[2].q, 3: self.con[3].q, 4:self.con[4].q, 5:self.con[5].q}
        #test packet-out
        ...
        # set swid
        for swid, sw in enumerate(switches, start = 1):
            self.con[swid].controller.table_add("swid", "set_swid", [], [str(swid)])
