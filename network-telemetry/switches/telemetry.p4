...
//compile the telemetry.p4 file by: p4c-bm2-ss --p4v 16 --p4runtime-files build/telemetry.p4info.txt -o build/telemetry.json telemetry.p4

#define CPU_PORT 255
// then start p4_switch by: sudo simple_switch_grpc -i 1@eth1 -i 2@eth2 -i 3@eth3 -i 4@eth4 --pcap pcaps --nanolog ipc:///tmp/s1-log.ipc --device-id 1 build/telemetry.json --log-console --thrift-port 9090 -- --grpc-server-addr 0.0.0.0:50051 --cpu-port 255  (the cpu-port in the start command must be the same, as it is the parsed value in the function MyParser below)
