...
class SimpleSwitchP4RuntimeAPI:
    ...
    ## Tables
    @handle_bad_input
    def table_add(self, table_name, action_name, match_keys, action_params=[], prio=0, rates=None, pkts=None, byts=None, idle_timeout=0):
        ...
        Args:
            ...
            byts (int)          : number of bytes to write (if **None**, the count
                                  is not changed)
            idle_timeout (int)  : idle timeout of a rule in nanosecond, see 
        ...
        # idle_timeout
        if idle_timeout:
            print('idle_timeout: {}'.format(idle_timeout))
            entry.idle_timeout_ns = idle_timeout
    ...
    @handle_bad_input
    def table_modify_match(self, table_name, action_name, match_keys, action_params=[], prio=0, rates=None, pkts=None, byts=None, idle_timeout = 0):
    ...
    @handle_bad_input #abC
    def packet_out(self, payload, egress_port):
        po = api.PacketOut(self.client, self.context)
        po.payload = payload
        po.metadata['egress_port'] = egress_port
        po.send()

    @handle_bad_input #abC
    def sniff_packet_in(self):
    #def sniff_packet_in(self, function=None, timeout=None):
        pi = api.PacketIn(self.client, self.context)
        return pi.packet_in_queue
        #pi.sniff(function, timeout)
        #print("sniff_packet_in:3")

    @handle_bad_input #abC
    def sniff_timeout_notification(self):
        ton = api.IdleTimeoutNotification(self.client, self.context)
        return ton.notification_queue

    @handle_bad_input #abC
    def read_table_normal_entries(self, table_name_str):
        #ref: https://github.com/jafingerhut/p4-guide/blob/master/testlib/p4runtime_shell_utils.py
        tes = [] 
        def do_save_te(te):
            tes.append(te)
        api.TableEntry(self.client, self.context, table_name_str).read(lambda te: do_save_te(te))
        return tes

    @handle_bad_input #abC
    def read_table_default_entries(self, table_name_str):
        #ref: https://github.com/jafingerhut/p4-guide/blob/master/testlib/p4runtime_shell_utils.py
        te = api.TableEntry(self.client, self.context, table_name_str)
        te.is_default = True
        for x in te.read():
            default_entry = x
    ...
    @handle_bad_input #abC
    def read_all_table_entries(self, table_name_str):
        return self.read_table_normal_entries(table_name_str), \
                self.read_table_default_entries(table_name_str)

    @handle_bad_input #abC
    def dump_table(self, table_name_str):
        entries, default_entry = self.read_all_table_entries(table_name_str)
        print(f"Table {table_name_str} contains {len(entries)} entries")
        for e in entries:
            print(str(e))
        print(f"Table {table_name_str} default entry:")
        print(default_entry)

    @handle_bad_input #abC
    def delete_all_normal_table_entries(self, table_name_str):
        print(f"Delete all normal entries in table {table_name_str}")
        api.TableEntry(self.client, self.context, table_name_str).read(lambda te: te.delete())
