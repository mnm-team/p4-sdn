...
We implemented the support for packet-in and packet-out mechanisms in the `p4utils` library (as of January 2024, the original p4utils library at https://github.com/nsg-ethz/p4-utils/tree/master/p4utils does not support these). The modification of the original p4utils library is made in the files `p4utils/utils/sswitch_p4runtime_API.py` and `p4utils/utils/p4runtime_API/api.py`.
