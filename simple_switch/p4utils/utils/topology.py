...
class InvalidServerIP(Exception):
    """Exception raised when a wrong server IP is provided."""
    def __init__(self, ip):
        self.message = "".format(ip)
        super(InvalidServerIP, self).__init__('InvalidServerIP: {}'.format(self.message))
...
class NetworkGraph(nx.Graph):
    ...
    def _populate_dicts(self):
        ...
        for node in self.nodes:
            ...
            if self.isServer(node):
                ip = self.nodes[node].get('ip')
                if ip is not None:
                    self.ip_to_server[ip.split("/")[0]] = {'name': node}
                    self.ip_to_server[ip.split("/")[0]].update(self.nodes[node])
    ...
    def get_servers(self, fields=[]):
        """Retrieves all servers and their configuration parameters.
        ...
        Returns:
            dict: all servers configuration parameters if ``fields`` is empty, or
            a dictionary of tuples, where the first one is indexed by server name and
            the second ones collect the values of the selected parameters in the exact
            order they are placed in ``fields``.
        ...
        Note:
            ...
            the value is directly put in the dictionary.
        """
        servers_subgraph = nx.subgraph_view(self, filter_node=self.isServer)
        servers = dict(servers_subgraph.nodes)
        if len(fields) > 0:
            reduced_servers = {}
            for server in servers:
                reduced_servers[server] = []
                for field in fields:
                    reduced_servers[server].append(servers[server].get(field))
                reduced_servers[server] = tuple(reduced_servers[server])
                if len(fields) == 1:
                    reduced_servers[server] = reduced_servers[server][0]
    ...
    def isServer(self, name):
        ...
        """
        self.checkNode(name)
        return self.get_nodes()[name].get('isServer', False)
    ...
    def get_server_name(self, ip):
        """Gets the server name from an IP address.

        Args:
            ip: server IP without prefix length

        Returns:
            str: name of the server whose IP corresponds to the one provided.

        Raises:
            InvalidServerIP: if the IP provided does not match any server.
        """
        server = self.ip_to_server.get(ip)
        if server:
            return server['name']
        else:
            raise InvalidServerIP(ip)
    ...
    def get_server_first_interface(self, name):
        """Gets the first interface of a server. 
        ...
        Returns:
            str: interface name.
        """
        if self.isServer(name):
            return list(self.node_to_intf[name].keys())[0]
        else:
            raise TypeError('{} is not a server.'.format(name))
    ...
    def get_server_ip(self, name):
        ...
        Warning:
            This method assumes that servers are single-homed (i.e.
            with only one interface).
        """
        if self.isServer(name):
            ip = self.get_nodes()[name].get('ip')
    ...
    def get_server_mac(self, name):
        """Gets server's MAC address.

        Args:
            name (str): server name

        Returns:
            str: server's MAC address.
        ...
        """
        self.checkNode(name)
        intf = self.get_server_first_interface(name)
        return self._node_interface(name, intf)['addr']
    ...
    def get_servers_connected_to(self, name):
        """Gets servers directly connected to a node.

        Args:
            name (str): node name

        Returns: 
            list: list of servers names.
        """
        self.checkNode(name)
        nodes = self.get_neighbors(name)
        return [server for server in nodes if self.isServer(server)]
