...
# the comment abC means "added by Cuong" (cuongtran@mnm-team.org), modifiations are carried out
# to the existing implementation available at 
# https://github.com/p4lang/p4runtime-shell/blob/main/p4runtime_sh/shell.py
...
def _sub_pkt_md(field, value, pcontext):
    ...
    return pcontext.context.get_packet_metadata_name_from_id(ctrl_pkt_md_name, id_)
...
class TableEntry(_P4EntityBase):
    ...
    **Usage**
    ...
    def __init__(self, client, context, table_name=None):
        ...
        table = self.context.get_table(table_name)
...
class PacketIn(): #from p4runtime-shell/p4runtime_sh/shell.py #abC

    def __init__(self, client, context):
        self.client = client #abC
        self.context = context #abC
        ctrl_pkt_md = P4Objects(P4Type.controller_packet_metadata, self.context)
        self.md_info_list = {}
        ...
        def _packet_in_recv_func(packet_in_queue, client):
            while True:
        ...
        self.recv_t = Thread(target=_packet_in_recv_func, args=(self.packet_in_queue, self.client))
        self.recv_t.start()
...
class PacketOut: #from p4runtime-shell/p4runtime_sh/shell.py #abC
    def __init__(self, client, context, payload=b'', **kwargs):
        self.client = client #abC
        self.context = context #abC

        self.p4_info = P4Objects(P4Type.controller_packet_metadata, self.context)["packet_out"]
...
class IdleTimeoutNotification:
    def __init__(self, client, context):
        ...
        def _notification_recv_func(notification_queue):
            while True:
                msg = self.client.get_stream_packet("idle_timeout_notification", timeout=None)
                if not msg:
                    break
